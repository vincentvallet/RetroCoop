...
export async function joinSession(sessionId:string,userId:string){
  const result=await serializable(async tx=>{
    ...
    const participation=await tx.sessionParticipant.upsert({where:{sessionId_userId:{sessionId,userId}},create:{sessionId,userId,status:'JOINED',version:1},update:{status:'JOINED',joinedAt:new Date(),version:{increment:1}}});
    ...
    return{row,email:{notificationId:notification.id,sessionId,participantId:userId,participantName:actorName,hostId:session.host.id,hostName:publicUsername(session.host.username,session.host.id),hostEmail:session.host.email,gameTitle:session.game.title,startsAt:session.startsAtUtc,timezone:session.timezoneAtCreation,participantCount:session.participants.length+1,maximumCapacity:session.maxPlayers,participationVersion:participation.version,enabled:session.host.joinEmailEnabled}};
  });
...
export async function leaveSession(sessionId:string,userId:string){
  const result=await serializable(async tx=>{
    const session=await tx.gameSession.findUnique({where:{id:sessionId},include:{participants:{where:{status:'JOINED'},select:{userId:true}},game:{select:{title:true}},host:{select:{id:true,email:true,username:true,leaveEmailEnabled:true}}}});if(!session)fail(404,'Session introuvable.','NOT_FOUND');if(session.hostId===userId)fail(409,'L’organisateur doit annuler ou supprimer sa session.','OWNER_CANNOT_LEAVE');
    if(!session.participants.some(item=>item.userId===userId))fail(409,'Vous ne participez pas à cette session.','NOT_JOINED');
    const [participation,actor]=await Promise.all([tx.sessionParticipant.update({where:{sessionId_userId:{sessionId,userId}},data:{status:'LEFT',version:{increment:1}}}),tx.user.findUniqueOrThrow({where:{id:userId},select:{id:true,username:true}})]);if(session.status==='FULL'&&session.startsAtUtc>new Date())await tx.gameSession.update({where:{id:sessionId},data:{status:'OPEN'}});
    const actorName=publicUsername(actor.username,actor.id),date=new Intl.DateTimeFormat('fr-FR',{dateStyle:'medium',timeStyle:'short',timeZone:session.timezoneAtCreation}).format(session.startsAtUtc),notification=await tx.notification.create({data:{recipientId:session.hostId,actorId:userId,sessionId,type:'SESSION_PARTICIPANT_LEFT',title:'Un joueur a quitté votre session',body:`${actorName} a quitté votre session ${session.game.title} du ${date}.`}}),row=await tx.gameSession.findUniqueOrThrow({where:{id:sessionId},include:sessionInclude});
    return{row,email:{notificationId:notification.id,sessionId,participantId:userId,participantName:actorName,hostId:session.host.id,hostName:publicUsername(session.host.username,session.host.id),hostEmail:session.host.email,gameTitle:session.game.title,startsAt:session.startsAtUtc,timezone:session.timezoneAtCreation,participantCount:session.participants.length-1,maximumCapacity:session.maxPlayers,participationVersion:participation.version,enabled:session.host.leaveEmailEnabled}};
  });
  return{session:serializeSession(result.row,userId),email:result.email};
}
...
export async function removeParticipant(sessionId:string,targetUserId:string,ownerId:string){
  return serializable(async tx=>{const session=await tx.gameSession.findUnique({where:{id:sessionId},include:{participants:{where:{status:'JOINED'},select:{userId:true}},game:{select:{title:true}}}});if(!session)fail(404,'Session introuvable.','NOT_FOUND');if(session.hostId!==ownerId)fail(403,'Vous n’êtes pas autorisé à modifier cette session.','FORBIDDEN');if(targetUserId===session.hostId)fail(409,'L’organisateur ne peut pas être retiré.','OWNER_CANNOT_BE_REMOVED');if(!session.participants.some(item=>item.userId===targetUserId))fail(404,'Participant introuvable.','PARTICIPANT_NOT_FOUND');await tx.sessionParticipant.update({where:{sessionId_userId:{sessionId,userId:targetUserId}},data:{status:'REMOVED',version:{increment:1}}});await tx.notification.create({data:{recipientId:targetUserId,actorId:ownerId,sessionId,type:'SESSION_PARTICIPANT_REMOVED',title:'Vous avez été retiré d’une session',body:`Vous avez été retiré de la session ${session.game.title}.`}});if(session.status==='FULL'&&session.startsAtUtc>new Date())await tx.gameSession.update({where:{id:sessionId},data:{status:'OPEN'}});return tx.gameSession.findUniqueOrThrow({where:{id:sessionId},include:sessionInclude})}).then(row=>serializeSession(row,ownerId));
}
