...
export const sessionInclude={
  ...
  participants:{where:{status:'JOINED' as const},orderBy:{joinedAt:'asc' as const},select:{userId:true,joinedAt:true,user:{select:{id:true,username:true,email:true}}}}
...
export function maskedEmail(email:string){const [name,domain='']=email.split('@');return `${name.slice(0,1)}***@${domain}`}
...
export function serializeSession(session:SessionWithRelations,currentUserId?:string|null){
  const participantIds=session.participants.map(item=>item.userId),isOwner=currentUserId===session.hostId,isCurrentUserParticipant=Boolean(currentUserId&&participantIds.includes(currentUserId));
  return{id:session.id,gameId:session.gameId,creatorId:session.hostId,status:session.status,visibility:session.visibility,startsAt:session.startsAtUtc.toISOString(),createdAt:session.createdAt.toISOString(),durationMinutes:session.durationMinutes,timezone:session.timezoneAtCreation,locationType:session.locationType,message:session.message,minPlayers:session.minPlayers,maxPlayers:session.maxPlayers,maxParticipants:session.maxPlayers,participantCount:session.participants.length,isOwner,isCurrentUserParticipant,canJoin:Boolean(currentUserId&&!isOwner&&!isCurrentUserParticipant&&session.status==='OPEN'&&session.startsAtUtc>new Date()&&session.participants.length<session.maxPlayers),game:session.game,creator:{id:session.host.id,username:session.host.username||maskedEmail(session.host.email)},participants:session.participants.map(item=>({userId:item.userId,username:item.user.username||maskedEmail(item.user.email),joinedAt:item.joinedAt.toISOString(),isOrganizer:item.userId===session.hostId}))};
}
