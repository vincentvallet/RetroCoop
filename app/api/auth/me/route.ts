...
export async function GET(request: Request) {
  ...
  return Response.json({authenticated:Boolean(user),user:user?{id:user.id,username:publicUsername(user.username,user.id),role:user.role,joinEmailEnabled:user.joinEmailEnabled}:null},{headers:{'Cache-Control':'no-store'}});
}
