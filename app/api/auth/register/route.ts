...
export async function POST(request:Request){
  ...
  if(!/^\S+@\S+\.\S+$/.test(email)||email.length>254||username.length<3||username.length>32||/^\S+@\S+\.\S+$/.test(username)||password.length<1||password.length>128||!input.challengeToken)return Response.json({error:'Champs invalides.'},{status:400});
  ...
  try{
    ...
    return Response.json({ok:true,user:{id:user.id,username:user.username}},{status:201,headers:{'Set-Cookie':sessionCookie(createSessionToken(user.id))}});
  }catch{
