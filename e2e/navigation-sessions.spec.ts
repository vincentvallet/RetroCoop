...
test('session : réponse 201, PostgreSQL, listes, rechargement et reconnexion',async({page})=>{
  ...
  await expect(page).toHaveURL(new RegExp(`/sessions/${created.session.id}`));await expect(page.getByText('Vous organisez cette session.')).toBeVisible();await page.reload();await expect(page.getByText('Vous organisez cette session.')).toBeVisible();
